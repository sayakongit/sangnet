...
from accounts.models import APPLICATION_STATUS_CHOICES
                ...
                if Donor.objects.filter(user=user).exists():
                    ...
                    for status_tuple in APPLICATION_STATUS_CHOICES:
                        if status_tuple[0] == application_status:
                            ...
                            break
                    return Response({'message': 'You have already applied as a donor', 'status': application_status}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def post(self, request):
        try:
            ...
            if not superuser.is_superuser:
                return Response({'message': 'You are not authorized to verify donors'}, status=status.HTTP_401_UNAUTHORIZED)
